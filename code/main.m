...
for k = 1:N
    ...
    [k1, l1] = f(state, vel, con);
    [k2, l2] = f(state + T/2*k1, vel + T/2*l1, con);
    [k3, l3] = f(state + T/2*k2, vel + T/2*l2, con);
    [k4, l4] = f(state + T*k3, vel + T/3*l3, con);

    G(:,:,k+1) = state + T/6*(k1 + 2*k2 + 2*k3 + k4);
    V(:,k+1) = vel + T/6*(l1 + 2*l2 + 2*l3 + l4);
end

ff = Function('ff', {U,P,D})
