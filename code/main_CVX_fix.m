...
%Q = 1000;
Q = diag([1,1,1,1000,1,1,1,1000,1,1,1,1000,1,1,1,0]);
R = diag([1,1,1,10,10,10]);
timestep = t(2) - t(1);

% MPC
load('lindyn_cor.mat')
while(mpciter < length(t)-2)
    
    % perturbation

    rot_pert = 0.0001 * randn(3,1);
    trs_pert = 0.0005 * randn(3,1);
    ...
    for i = mpciter+1:mpciter+N+6
        g_des(:,:,i-mpciter+1) = xd(:,:,i);
        v_des(:,i-mpciter+1) = vd(:,i);
    end
        
    % obtain A and B, and SE3 dynamics structure
    [c, r] = invSE3(x0);
    ...
    % introduce variables:
        ...
        for i = 1:N-1
            ...
            % Linearized dynamics (only translational)
            
            ldyntemp = A * xtemp(10:end,end) + B * u(:,i);
            veldyn = ldyntemp(4:9) * timestep;
            %statedot = SE3(BS_temp, R_temp) * se3alg(xtemp(13:end,end));
            %BSdyn = BS_temp * CrossProd(xtemp(13:15,end) * timestep);
            BSdyn = [xtemp(15,end) * xtemp(4) - xtemp(14,end) * xtemp(7);
                xtemp(15,end) * xtemp(5) - xtemp(14,end) * xtemp(8);
                xtemp(15,end) * xtemp(6) - xtemp(14,end) * xtemp(9);
                xtemp(13,end) * xtemp(7) - xtemp(15,end) * xtemp(1);
                xtemp(13,end) * xtemp(8) - xtemp(15,end) * xtemp(2);
                xtemp(13,end) * xtemp(9) - xtemp(15,end) * xtemp(3);
                xtemp(14,end) * xtemp(1) - xtemp(13,end) * xtemp(4);
                xtemp(14,end) * xtemp(2) - xtemp(13,end) * xtemp(5);
                xtemp(14,end) * xtemp(3) - xtemp(13,end) * xtemp(6);] * timestep;

            Rdyn = xtemp(16:18,end) * timestep;

            dyntemp = [reshape(BSdyn, [], 1); Rdyn; veldyn];
            xtemp = [xtemp, xtemp(:,end) + dyntemp];
        ...
        obj = obj + reshape(h_temp, [], 1)' * Q * reshape(h_temp, [], 1);
    ...
    % timestep
    timestep = t(mpciter+2) - t(mpciter+1);
